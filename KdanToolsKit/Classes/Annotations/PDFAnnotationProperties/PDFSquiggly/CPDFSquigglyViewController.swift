//
//  CPDFSquigglyViewController.swift
//  ComPDFKit_Tools
//
...
@objc protocol CPDFSquigglyViewControllerDelegate: AnyObject {
    @objc optional func squigglyViewController(_ squigglyViewController: CPDFSquigglyViewController, annotStyle: CAnnotStyle)
}

class CPDFSquigglyViewController: CPDFAnnotationBaseViewController {
    weak var delegate: CPDFSquigglyViewControllerDelegate?
    ...
    override func commomInitTitle() {
        titleLabel?.text = NSLocalizedString("Squiggly", comment: "")
    ...
    override func selectColorView(_ select: CPDFColorSelectView, color: UIColor) {
        ...
        if annotStyle != nil {
            delegate?.squigglyViewController?(self, annotStyle: annotStyle!)
        }
